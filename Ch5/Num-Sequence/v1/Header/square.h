...
public:
	Square(size_t beg_pos = 1, size_t len = 1) : _beg_pos(beg_pos), _len(len) {
		if (_beg_pos < 1 || len < 1) {
			cerr << "Square constructor: invalid arguement, set to 1" << endl;
			_beg_pos = _len = 1;
		}
	...
	unsigned long int elem(int pos) {
		...
		{
			cerr << "Square::elem() Invalid argument" << endl;
			return -999;
		}
...
vector<unsigned long int> Square::_elems_vec;
